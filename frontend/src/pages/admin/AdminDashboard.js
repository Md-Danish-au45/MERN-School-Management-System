  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      <Drawer
        variant="permanent"
        open={open}
        sx={open ? styles.drawerOpen : styles.drawerClose}
      >
        <Toolbar sx={styles.drawerToolbar}>
          <IconButton onClick={toggleDrawer}>
            <ChevronLeftIcon sx={{ fontSize: 28, color: "#ffffff" }} />
          </IconButton>
        </Toolbar>
...
const styles = {
  ...
  // AppBar styling with a unique gradient, rounded bottom corners, and smooth shadow
  appBar: {
  ...
  // Toolbar inside AppBar with added padding and spacing
  headerToolbar: {
    pr: 4,
    minHeight: 64,
    display: "flex",
  ...
  // Styles for the open drawer (permanent) version
  drawerOpen: {
    width: 240,
    flexShrink: 0,
    "& .MuiDrawer-paper": {
  ...
  // Styles for the closed drawer when screen is larger (hiding it on extra small screens)
  drawerClose: {
    width: 64,
    flexShrink: 0,
    "& .MuiDrawer-paper": {
  ...
  // Toolbar styling for drawer header area with dark background and inner shadow
  drawerToolbar: {
    display: "flex",
    alignItems: "center",
